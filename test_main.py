...
def test_post_transactions_happy_path():
    ...
    expected = [
        {'date': '2020-07-01', 'category': 'Expense', 'amount': '18.77', 'memo': 'Fuel§'},
        {'date': '2020-07-04', 'category': 'Income', 'amount': '40.00', 'memo': '347 Woodrow'}
        ]
...
def test_get_report_returns_errors():
    ...
    assert(400, response.status_code)
    expected = {'detail':
        'Cannot generate report because there are no stored transactions - use the /transactions endpoint to put some in.'}
    assert(response.json() == expected)
